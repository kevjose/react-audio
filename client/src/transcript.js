export const TRANSCRIPT = {
  ...
  word_timings: [
    ...
    [
      ...
      { startTime: '13.800s', endTime: '15.9s', word: 'good' },
    ],
  ],
};

export const filler = (word_timings) => {
  let filled = [];
  if (word_timings[0].startTime !== '0.000s') {
    filled.push({
      startTime: '0.000s',
      endTime: '0.100s', //word_timings[0].startTime,
      word: '',
    });
  }
  for (let i = 0; i < word_timings.length; i++) {
    let filledLen = filled.length;
    if (filled[filledLen - 1].endTime === word_timings[i].startTime) {
      filled.push(word_timings[i]);
    } else {
      filled.push({
        startTime: filled[filledLen - 1].endTime,
        endTime: word_timings[i].startTime, //`${parseFloat(filled[filledLen - 1].endTime) + 0.1}s`, //word_timings[i].startTime,
        word: '',
      });
      i = i - 1;
    }
